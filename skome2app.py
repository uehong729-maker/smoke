...
with tab1:
    ...
    if not st.session_state.check_in:
        if st.button("今日打卡 ✅", key="checkin_btn"):
            ...
            st.balloons()
            st.success("🎉 打卡成功！加20分！繼續堅持！")
    else:
        st.info("📌 今日已經打卡啦！聽日再嚟！")
        ...
        if st.button("❌ 拒絕", key="refuse_btn"):
            if correct == "拒絕":
                ...
                if st.session_state.points >= st.session_state.game_level * 50:
                    st.session_state.game_level += 1
                    st.success(f"🎉 通關！進入第 {st.session_state.game_level} 關！解鎖新獎勵！")
                    st.balloons()
            else:
...
with tab4:
    ...
    # 煙癮對抗小技巧
    st.subheader("🔥 想吸煙？即刻做呢幾件事")
    st.write("1. 嚼無糖口香糖 / 食薄荷糖")
    st.write("2. 做10次深呼吸，放鬆身心")
    st.write("3. 即刻離開現場，去做運動")
    st.write("4. 搵朋友傾計，轉移注意力")
    st.markdown('</div>', unsafe_allow_html=True)
